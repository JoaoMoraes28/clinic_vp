...
class Doctor(Base):
    ...
    crm: Mapped[str] = mapped_column(String(10), nullable=False)

    crm_uf_id: Mapped[int] = mapped_column(ForeignKey("uf.id"), nullable=False)
    ...
    status: Mapped[StatusDoctorRecepcionist] = mapped_column(
        SQLEnum(StatusDoctorRecepcionist), nullable=False
    )
    ...
    doctor_days: Mapped[list["DoctorDay"]] = relationship(back_populates="doctor")
    ...
    doctor_address: Mapped["DoctorAddress"] = relationship(back_populates="doctor")

    uf: Mapped["UF"] = relationship(back_populates="doctor_crm")
