...
class Recepcionist(Base):
    ...
    salary: Mapped[Decimal] = mapped_column(Numeric(6, 2), nullable=False)

    cpf: Mapped[str] = mapped_column(String(11), unique=True, nullable=False)

    status: Mapped[StatusDoctorRecepcionist] = mapped_column(
        Enum(StatusDoctorRecepcionist), nullable=False
    )
    ...
    gender: Mapped[GenderOption] = mapped_column(Enum(GenderOption), nullable=False)
